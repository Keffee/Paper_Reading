...
#### 20260707 关于OneReason相关的改进
- 

#### 20260528 我们Semantic ID做损失到底有什么做头？SID能跨域吗？——难
- 实际的三个问题：信息是否有所损失？损失的信息会不会影响推荐任务？分布偏移的代价是否太大？
- GRLM：使用自然语言关键词替代数值化的Semantic ID
- AgenticTagge：使用强模型构建词表结构、形成候选词库，然后弱模型负责选词
  - 强模型构建，弱模型尝试

#### 20260520 Relation Learning到底是什么
- 我们推荐的时候，能否出了单纯的对错之外，引入一种类似结构的信息呢？
  - 图距离度量奖励性？
  - 节点中心性度量长尾？
  - 根据Potential-Based Reward Shaping定理保证，只要辅助函数满足势函数性质，就能在不改变最优策略的前提下加速学习。
- 可能的技术路线：
  - 图拓扑结构能否传播终端奖励，从而生成中间状态的密集奖励？
    - 利用GCN的信号传播能力，把正信号传播到涂上的邻近物品，为推荐提供非零奖励（有点类似势函数，走向正确的方向就能有一定优势）
  - 知识图谱当成可验证奖励，路径质量作为中间反馈
  - 通过因果推断或结构化critic将序列级奖励分解为item/token级信号

#### 20260519 研究一下连续扩散语言模型
- 很长一段时间里，似乎离散扩散语言模型（在离散空间中定义扩散过程，预测被掩码的token来生成）拥有更强的性能，。中间的一个非常重要的贡献是Duo等人揭示离散扩散本质上可以从连续高斯扩散推导出来，后来有发现Embedded Language Flows使用更少的数据、无需蒸馏的方式，全面超越了离散扩散模型
- 何恺明的工作的一个重要思想——过去的扩散模型预测的是噪声$\epsilon$或者速度$v$，根据流形假设，自然数据位于低维流形上，一个容量有限的网络可以学习到这个低维结构；但噪声和速度都是全维的，需要无限容量才能精确表示。因此，x-prediction 在高维空间中本质上优于 ε-prediction 和 v-prediction。
- Flow Matching：$\mathbf{z}_t = t \cdot \mathbf{x} + (1 - t) \cdot \boldsymbol{\epsilon}, \quad t \in [0, 1]$
- CoDAR（2026）发现，对于token的离散化，自回归Transformer比逐位置现行分类强非常多，token之间强力的前后关系导致我们必须使用这种方法。但是代价太大了，这意味着两倍开销和不稳定，ELF发现共用一个共享权重就行了。

#### 20260514 研究一下harness有没有说法
- Agent=Model+Harness，Agent Harness 是包裹在 LLM 周围的完整软件基础设施层，包括编排循环（orchestration loop）、工具注册与调度（tool registry & dispatch）、上下文生命周期管理（context lifecycle management）、状态持久化（state persistence）、权限与安全模型（permissions & safety）、记忆系统（memory）、错误恢复（error recovery）和人机协作接口（human-in-the-loop）。它是使裸模型成为可靠工作 agent 的一切。
- MCP 是 AI Agent 的 USB-C：正如 USB-C 统一了设备连接标准，MCP 统一了 agent 与外部工具/资源的连接标准。而 harness 则是整台电脑——包含 USB-C 接口在内的完整系统。
- 工具的意义：工具要脱离LLM本身实现，用MCP来运行，这种感觉才是工具系统。然后只要根据MCP协议来设计，LLM怎么引入就不用管了，自动启动这块
- 上下文管理的意义：似乎上下文管理就是当代Harness的核心能力之一——上下文漂移会带来巨大的性能下降
- 似乎状态持久化和权限与安全很重要，但是似乎和我们的结合不是很强
- 记忆系统：不必多说，核心中的核心，分为短期记忆、长期记忆和外部记忆
- 错误恢复：可能包括重试、回退、自我修正和检查点的方面，并且可以引入对抗机制来实现循环改进
- 人机协同：应当有效判断什么时间该寻求帮助，通过信任的建立来逐步减少人工干预频率
- 看得出没太大说法了

#### 20260512 AgenticRL也是LLM4RecRL吗
- 或许把LLM训练扩增到多轮工具调用等交互场景就算Agentic？
- 当前AgenticRL的核心问题：
  - 信用分配：如此高的操作量得到结果，轨迹内的信用（似乎是奖励）如何分配？
  - 训练崩溃：Agent学会了安全但无用策略，奖励整体低下；策略熵快速下降，模型过早收敛到次优
  - 长期规划效率：超长周期效率低，至少也要环境设计和奖励、策略都发力才行
- arxiv'2505, ARTIST — Agentic Reasoning and Tool Integration in Self-improving Transformers
  - 早期Agentic，考虑到了推理和工具调用
  - 将输出分为“内部推理、工具调用、工具输出和答案”四个part，特意给工具输出token掩码来避免它们参与loss计算
- arxiv‘2505, RAGEN — Understanding Self-Evolution in LLM Agents via Multi-Turn Reinforcement Learning
  - 发现echo trap（回声陷阱），模型在多轮Agent的RL中会突然崩溃，具有突发性、不可逆性、普遍性和隐蔽性
  - 也是本文提出的训练崩溃的两个问题，这两个问题其实都是倾向于了低熵的确定性次优结果，因此提出的方法也是找高方差信号来训练。此外的clip修改和KL散度修改是老生常谈。
- arxiv‘2505, WebAgent-R1 — Training Web Agents via End-to-End Multi-Turn Reinforcement Learning
  - 重点关注web导航的agent任务，这个任务也确实重要，因为网页具有token量大、变化快、动作精度高而复杂、奖励延迟高而不可逆的特点
  - GRPO似乎没改。然后长上下文进行了多层压缩
  - 单纯LLM不能直接用来RL训这个，还是需要先SFT激活一下这个能力
  - 发现在这种情况下，进行交互轮数提升似乎能有效提升成功率
- arxiv'2509, OPRL — Online Process Reward Learning for Agentic Reinforcement Learning
  - 信用分配问题难以解决，要么轨迹级奖励太粗，要么就得设计外部奖励模型代价太大
  - DPO证明了最优策略和奖励函数之间存在对数概率比的关系，这里改为奖励“新模型下生成概率超过旧模型”的action；同时也保留整个过程的奖励，使用DPO风格来区分
- NeurIPS'26, A Practitioner's Guide to Multi-turn Agentic Reinforcement Learning
  - 比较系统地实验了到底什么样的AgenticRL方法算好，设定的任务比较像web导航任务
  - 有很多实验结论，这里仅记录一些看起来比较有趣的
    - turn-level的奖励似乎比outcome-only奖励和sep奖励都好，horizon长了之后outcome几乎没法收敛
    - 复杂任务上LLM-as-judge好像效果还不如规则评估
    - 一般GRPO效果优于PPO，GRPO似乎善于利用outcome奖励，PPO善于利用step-level奖励
    - 但是在10步以内的超短步情况下，单纯outcome+GRPO+无上下文管理甚至最佳，超高随机性和长交互才是turn-level奖励+PPO+LLM摘要最佳
- ICLR'2026, LaMer — Meta-RL Induces Exploration in Language Agents
  - Agent似乎会在每个episode内部进行优化，当跨出当前任务的时候好像就会比较烂，能不能鼓励模型跨episode积累各种信息。
  - 即使当前的episode中动作较差，奖励较低，如果它对于未来的所有episode来说是有利的，那奖励整体还是会比较高（牺牲前期奖励换取后期奖励）
  - 每个episode之后都会调用Agent来生成一段文本的反思作为之后episode的输入
  - 对Pass@1的提升并不强，重点在于提升Pass@3，同时通过鼓励探索的形式，能让模型随着探索次数上升的效果更明显。
  - *这是否对强调Pass@1、只能推荐一次的推荐任务反而是一种损失？*
- arxiv‘2603, SLEA-RL — Step-Level Experience Augmented Reinforcement Learning for Multi-Turn Agentic Training
  - 也在说跨episode的经验积累问题，声称现在的工作都是在开始任务的时候进行经验检索，这和实际需求不符，实际上要求是非常动态的，前期和后期需要的经验可能完全不同（但是就算是cc也是过程中一直都会检索啊（
  - 因此，设计了一个经验保存和索引机制，有效组织记忆，提升检索速度，并且让每一步都进行检索
  - 轨迹会根据结果进行分类，并且根据奖励得到质量分数，根据质量分数决定经验是否存下来（这，这就叫自进化？）
  - 这篇和上一篇都在用一个叫WebShop的benchmark来评估，是一个模拟电商平台网页的数据集，信息是从亚马逊爬的，可以进行搜索和点击，并且用人类众包构建了多条演示轨迹，用来做优质查询、导航策略性探索、长程比较和记忆等问题
- arxiv'2603, Demystifying Reinforcement Learning for Long-Horizon Tool-Using Agents: A Comprehensive Recipe
  - 现有技术好像缺乏一个端到端的可复现训练流程来指导大家从零开始训练长周期工具调用agent
  - 长周期工具调用问题：上下文爆炸、错误传播、奖励稀疏、训练效率低
    - 上下文爆炸：使用分层摘要（cc做法，果然是好方法）
    - 奖励稀疏：拆解子目标作为中间奖励，并且设计工具奖励
    - 错误传播：允许回退，并且训练阶段就分阶段训练
    - 训练效率：只能异步+优质前缀轨迹复用
    - 评估：工具调用正确，中间子目标达成、最终完成情况和效率指标
  - 这样一看有点像实验技术报告，实验发现：
    - 分阶段训练非常关键，开始就做长程训练很难收敛
    - 引入中间奖励有效提升了样本效率（训练速度）
    - 分层摘要很重要，上下文压缩带来很高性能
    - 中间奖励可能需要递减，如果固定权重容易导致agent对中间目标过度优化，权重可以从1衰减到0.1
- arxiv'2604, From Reasoning to Agentic: Credit Assignment in Reinforcement Learning for Large Language Models
  - 综述，似乎专门考虑信用分配问题
  - 工具调用Agent在信用分配方面的问题：*序列过长*、*奖励稀疏*、*因果关系复杂*（token质量取决于上下文）
  - 从粒度角度分类：
    - 轨迹级：实现简单，标准GRPO就行，但是太稀疏了，也无法区分步骤之间的重要性
    - 回合级：代表方法GTPO，对每个交互回合进行奖励估计
    - 步骤级：每个推理步都计算，但是奖励又怎么来呢，额外奖励模型？在线的某种评估手段？
    - token级：不但成本高，而且噪声大，实践没人用
  - 从方法论角度分类：
    - 基于模型的：利用显式的价值函数或者奖励模型来估计价值，如PPO，稳定但是非常依赖额外训练
    - 基于对比的：基于成功失败的轨迹对比推断信用，比较像DPO的变体，这是否依赖轨迹多样性？
    - 基于因果的：利用反事实采样推理来确定因果贡献（修改这一步会带来什么改变？），但是强依赖环境
    - 基于注意力权重：利用模型内部注意力模式来估计token/步骤重要性
  - 最终性能对比
    - 环境噪声使得似乎步骤级与轨迹级之间的差距没那么大了
    - 回合级方法在超高随机性环境中抗干扰能力很强，可能具有其优势
    - 基于对比的方法和基于模型的方法可能更实用一点
- arxiv'2604, Rethinking Agentic Reinforcement Learning In Large Language Models
  - 当前Agentic RL存在三个局限：
    - 静态目标：现在的奖励相对固定，但是真实情况下人类会根据现状自行降级或者升级目标
    - episodic交互，难以跨episode积累认知经验
    - 缺乏自我意识：模型不知道自己的能力边界，因此有时不会主动寻求外部帮助
  - 核心方法：
    - Meta-Reasoning：训练模型识别自身推理过程质量的能力，动态识别推理中的错误、动态调整推理深度、自行选择推理策略
    - Self-Reflection：失败后自动归因、生成可复用的经验、后续任务中自行回忆
    - Adaptive Goal Setting：自适应拆解目标，构建层级化奖励，并且根据目标动态调整后续计划
- 那我们呢？
  - 信用分配问题：如何确定整条轨迹中每个token对最终结果的贡献？
    - AgenticRL：多步交互，哪个重要？有可能通过回合级评估来确认，也有可能利用一个在线学习算法来估计每一步的价值函数
    - OpenOnerec：实际上只有一步：生成4个token，如果能用多目标来构造的话，现在面临的问题就是“什么token最关键”层级信息也非常类似于AgenticRL里面的先后调用关系
  - 过程奖励问题：最终奖励很稀疏的时候如何为中间过程构造有意义的学习信号？
    - 没找到太多能做的，多样性奖励？前缀奖励？
  - 训练稳定性：避免训练崩溃
    - 两边都是那一套：熵正则化、奖励归一化、课程学习
  - 探索问题：如何找到长尾item？
    - AgneticRL需要鼓励跨episode的探索，甚至把“探索”当成一项能力来训练
    - 推荐里面GFlowGR能够通过loss函数将输出的分布推向奖励的分布——只要根据我们的期望设计奖励，就能让输出贴近那个分布（适配的是分叉奖励、目标奖励、CTR奖励、多目标奖励）
  - 经验增强：如何跨过多个样本积累知识？
    - 同一个用户能不能推荐多次呢？前几次探索不同兴趣，后面变成精准推荐
    - 维护一个memory，能行吗？利用memory增长序列，知识注入，总之为了显式引入知识
  - 认知诊断：模型能否理解当前的决策质量
    - 现阶段只能通过置信度和模型显式输出来确认，好像有点难做


#### 20260205 RL&LLM 请
- Reinforcement Fine-tuning Naturally Miti-gates Forgettingin Continual Post-training
- [A Survey of Data Processing for LLM(-Agent): Datasets, Techniques and Practices](https://github.com/weAIDB/awesome-data-llm)
  - 分析了预训练、持续预训练、SFT微调、强化学习、RAG和Agent六个阶段需求的数据集情况
    - 1. 预训练：超大规模T级以上token、来源广泛、类型多样、模态丰富
    - 2. 持续预训练：10M-100B级token到T级数据来填补知识空白、适应特定领域
    - 3. SFT微调：大概100K个到10M个示例，，数据量小但是质量要高，有通用指令遵循和特定领域对齐两种
    - 4. 强化学习：
      - 1. RLHF：更少，1K-1M示例，多见偏好数据，偏好数据一般都是人标或者强大模型来对结果排序
      - 2. RoRL（推理导向的强化）：经典GRPO系，一个简单又强的反馈信号是关键（比如数学答案）
        - 现在能够有一种感觉：GRPO更加擅长的是基于客观答案给出一定性分数，分数主要是为了优化思维链质量而存在，如果把reward设计成一个model反而似乎降低效果（RM能力有限/同一组样本生成差异不大）
    - 5. RAG：利用检索保证真实性和有效性
    - 6. Agent：提升规划、工具编排和多轮对话能力
  - 主要包含数据采集、数据去重、数据过滤、数据选择、数据混合和数据合成
    - 1. 数据采集：基本从网页等结构文本或者手写等复杂噪声中提取文档
    - 2. 数据去重：降低过拟合提升泛化性；通过URL匹配、精确字符串匹配、哈希识别、基于嵌入的聚类和样本重加权
    - 3. 数据过滤：去除低质量数据和有毒数据
      - 1. 样本级过滤：困惑度测量、影响评估、聚类、模型评分和混合策略
      - 2. 内容级过滤(安全?)：无意义样本、语言、有毒语言、个人可识别信息和不当图像视频
    - 4. 数据选择：评估子数据集并评估它们代表目标分布的能力，所以基于相似度、基于优化和基于模型
      - 使用先进的方法要耗费显著提升的时间，但是未必能带来效果提升，理论上应该减少了过拟合和提升鲁棒性，但是实际上可能造成特殊样本过采样，有研究发现重复0次或多次的样本可能质量较低
    - 5. 数据混合：直接调整、设计双层优化、鲁棒优化、构建性能预测模型来优化。带来了跨领域泛化能力，但由于非线性缩放效应，小模型或特定架构的最佳比例不一定适合更大的模型或不同的架构
      - 1. 训练前基于人类经验混合
      - 2. 训练前基于模型优化
      - 3. 训练期间进行双层优化
      - 4. 训练期间考虑分布鲁棒性（平滑、快速响应难度变化）
    - 6. 数据合成与精炼：
      - 1. 基于 领域知识、指令跟随、智能体、图文能力 来合成多样且一致的数据
      - 2. 隐私保护合成和重写来保护隐私
      - 3. 引导推理链、添加step级注释、自循环反思 来生成逐步推理数据
      - 4. 利用大模型来偏好反馈
    - 7. 端到端处理pipeline
      - 大致遵循三个核心原则：扩展性、严格去重和启发式过滤
  - 未来方向：pipeline优化、知识更新和版本控制、综合数据集评估
- arXiv'26, [TokenMixer-Large: Scaling Up Large Ranking Models in Industrial Recommenders](https://www.alphaxiv.org/abs/2602.06563)(快手)
  - 每2-3层增加一些残差连接
  - 从低层输出一个轻量的辅助损失来和高层预测结合
  - 数值容易爆炸，将后置归一化改为前置归一化
  - 引入MoE，首先训练的时候用一个超大的SwiGLU全连接层，然后根据全连接层切分为E个小expert（将D变为nD的时候，n除以E），保留一个共享专家
- arXiv'26, [Bending the Scaling Law Curve in Large-Scale Recommendation Systems](https://arxiv.org/html/2602.16986v2)(Meta)(ULTRA-HSTU)
  - 解决HSTU面对超长序列的计算质量和计算效率问题
  - 技术一：前N_1层保留完整长序列，后N_2层只保留较短的片段（LongFormer思路，常见于长序列）。同时对多种行为处理成多条独立序列并行处理。
  - 技术二：改进注意力机制，首先每个token只关注前K1个token，形成滑动窗口；最新的K2个latest token对所有token都关注
  - 技术三：超绝底层优化，改进量化技术和算子优化
  - 效果上看训练和推理速度都极大扩张
- 26, [Integrating Large Language Models with Reinforcement Learning: A Survey of LLM-RL Synergistic Recommendation](https://www.techrxiv.org/users/1027900/articles/1387604-integrating-large-language-models-with-reinforcement-learning-a-survey-of-llm-rl-synergistic-recommendation)(快手)(人大)(何组)
- WWW'26, [Does LLM Focus on the Right Words? Mitigating Context Bias in LLM-based Recommenders](https://arxiv.org/pdf/2510.10978)
  - 似乎是在LLM as Rec做SFT的时候给“与辅助的描述类token相距较远”的target加权？
- arXiv'26, [Spend Search Where It Pays: Value-Guided Structured Sampling and Optimization for Generative Recommendation](https://arxiv.org/html/2602.10699v2)(Tencent)(腾讯)(V-STAR)
  - 传统的奖励设计存在奖励错配，高奖励的物品可能前缀概率低，直接被beam search剪枝掉。

#### 20260131 紧急调研token粒度RL相关工作
- 0. 那些可能可以参考的论文：
  - Reasoning with Exploration: An Entropy Perspective
  - Let’s Verify Step by Step
  - [Entropy-Based Advantage Shaping](https://www.emergentmind.com/topics/entropy-based-advantage-shaping-mechanism)
  - TIS-DPO: TOKEN-LEVEL IMPORTANCE SAMPLING FOR DIRECT PREFERENCE OPTIMIZATION WITH ESTIMATED WEIGHTS
  - Token-Importance Guided Direct Preference Optimization
  - T-REG: Preference Optimization with Token-Level Reward Regularization
  - [Token-Level Optimization Framework](https://www.emergentmind.com/topics/token-level-optimization-framework)
  - Attention-Based Reward Shaping for Sparse and Delayed Rewards
- 1. [GTPO and GRPO-S: Token and Sequence-Level Reward Shaping with Policy Entropy](https://alphaxiv.org/abs/2508.04349)
  - 利用熵（置信度）当成一种额外奖励，也就是说奖励更自信说出正确答案的，惩罚过度自信说出错误答案的
- 2. [KTAE: A Model-Free Algorithm to Key-Tokens Advantage Estimation in Mathematical Reasoning](https://alphaxiv.org/abs/2505.16826)
  - 基于每个token在所有正确/错误回复中的出现频率来判断token的正确性
  - 引入了费希尔精确检验和信息增益来判断token和正确性的关系（凑）
- 3. [Do Not Let Low-Probability Tokens Over-Dominate in RL for LLMs](https://alphaxiv.org/abs/2505.12929)
  - 发现低概率 token 在 RL 训练中往往会过度主导梯度，所以要加权
  - 最简单：直接给低概率token更低的权重
  - 分阶段更新：首先更新低概率组，然后更新高概率组
- 4. [ASPO: Asymmetric Importance Sampling Policy Optimization](https://alphaxiv.org/abs/2510.06062)
  - 传统GRPO会给高置信的正确答案高权重，这导致了熵坍塌
  - 拆解传统GRPO，的确梯度和旧策略采样分数成反比，如果能反转的话，就可以让RL变成一种难样本挖掘任务，数学上并非无偏，但是训练边际效益大
  - 但是翻转reward会出现正确token概率太低的时候reward无限大，所以裁剪一下
- 5. [TLCR: Token-Level Continuous Reward for Fine-grained Reinforcement Learning from Human Feedback](https://aclanthology.org/2024.findings-acl.889.pdf)
  - 比较老了，利用GPT-4作为修改器，为淘汰的回答微调，看要微调多少/看哪些token被改了，这样获得token级别的reward
- 6. [Exploiting Tree Structure for Credit Assignment in RL Training of LLMs](https://arxiv.org/html/2509.18314v2)
  - 模型的一组采样本质是一个前缀树，在每个分叉点，理论可以专门计算分叉点对最终结果的贡献度
- 7. [RED: Unleashing Token-Level Rewards from Holistic Feedback via Reward Redistribution](https://arxiv.org/html/2411.08302v1)
  - 把奖励任务视作一个回归任务，评估每一个新增token对RM最终得分的边际贡献
  - 问题是，我们没法计算每个位置的奖励...
  - 但如果用前面的方式得到奖励，就可以计算边界得分了（感觉大家都差不多）
- 8. [ResT: Reshaping Token-Level Policy Gradients for Tool-Use Large Language Models](https://openreview.net/pdf?id=gNZlaKRWki)
  - 计算每个token生成时的预测熵，根据信息熵来动态调整每个token的梯度权重
  - 由于本文的面向方向是推理关键token（工具调用），所以设计上会渐进式提高推理关键token的权重

#### 20251014 总之就是泛用推理相关
- 1. Arxiv'25, [Mixture-of-Visual-Thoughts: Exploring Context-Adaptive Reasoning Mode Selection for General Visual Reasoning](https://arxiv.org/pdf/2509.22746)
  - 模型如何统一多个不同的推理过程？
  - 本文几乎专为vlm设计，这可能使得难以迁移
  - 在vlm中，似乎输入可以是text，也可以是输入的图像，本文强调这两种输入的不同，因此针对不同的输入进行了不同的处理（n条TXT和n条GRD，然后分别计算优势函数，并且可以计算相对另一种输入的相对优势，这样利用相对优势就能计算出哪个模式好）
- 2. CIKM'24, [Bridging User Dynamics: Transforming Sequential Recommendations with Schrödinger Bridge and Diffusion Models](https://arxiv.org/pdf/2409.10522)（文嘉的cikm）
  - 传统扩散模型：将用户历史交互作为condition输入，然后从纯高斯噪声开始重建，信息损耗大
  - 用用户当前状态代替高斯噪声作为扩散初始分布，用薛定谔桥推导用户状态和目标物品间的最优转移路径
- 3. arXiv‘25, [I Think, Therefore I Diffuse: Enabling Multimodal In-Context Reasoning in Diffusion Models](https://arxiv.org/pdf/2502.10458)
  - 如何为扩散模型赋予多模态上下文理解与推理能力
  - 看起来训练过程平平无奇，当成传统vlm或者clip对齐就行，甚至没有用到diff模型
  - 而推理过程甚至只是把vlm的llm decoder换成了diff decoder
  - 主要作用是把VLMs的多模态上下文理解与推理能力转移到了扩散模型上。
- 4. arXiv '25, [Less is More: Recursive Reasoning with Tiny Networks](https://arxiv.org/pdf/2510.04871)
  - 使用迭代的小模型，推理效果能超过一众大模型吗？
  - 似乎对于模型输入来说，主要是输入x，之前的答案y和之前的推理（隐变量）z，这三个
  - 我们认为z是思维链，不带上z就会忘记y的到来方式
  - 增加层数，MoE，输入和输出嵌入绑定，只用后几步梯度，效果均下降
- 5. ArXiv '25, [MATT-CTR: Unleashing a Model-Agnostic Test-Time Paradigm for CTR Prediction with Confidence-Guided Inference Paths](https://arxiv.org/pdf/2510.08932)(阿里)(ali)
  - 看起来似乎是做CTR模型的 test time 特征选择？特征路径选择？
  - 第一步是层次哈希待输入特征，因为哈希能够计算频数，这样哈希表就能包含所有特征映射到当前桶的计数，不同的哈希策略得到的计数和组合方式就不同了，就能通过这个计数的最小值找到对应的“高频特征组合”，对于低频特征组合，由于每次哈希方式不同，就能展现出方差，利用切比雪夫不等式就能得到分数下界，当成每个特征组合的置信度
  - 第二步是选取特征路径，利用第一步得到的置信度计算方式，可以计算出在当前路径下，增加一个新的特征之后形成的特征组合的置信度，然后基于置信度来随机采样决定是否纳入路径，T步之后得到高质量组合一条。多来几次就有多几条。
- 6. ICLR '26在投[Base Models Know How To Reason, Thinking Models Learn When](https://arxiv.org/pdf/2510.07364)
  - 回答：推理模型为什么优于基础模型呢？
  - 提出并验证混合模型假设：模型本身就有推理能力，只是需要强化或者蒸馏来学会何时调用这个能力
- 7. arXiv'25, [GPR: Towards a Generative Pre-trained One-Model Paradigm for Large-Scale Advertising Recommendation](https://arxiv.org/pdf/2511.10138)（腾讯）
  - 在广告系统中引入了端到端生成式推荐，成功上线
  - 当前面临的问题：数据和行为异质性、效率和灵活性难两全、各种利益优化
  - 输入表示成4种令牌，user（用户属性和偏好）、organic（用户最近查看的短视频和文章）、environment（广告请求周围的上下文）、item（已互动的广告内容）
  - RQ-Kmeans+，竟然是首先使用RQ-Kmeans获得初始codebook表征，然后使用RQ-VAE来进行优化（这个创新点...暂且相信一下吧）
  - （HSD）混合注意力掩码，前缀中的块内token互相是可以看到的，同时大方向还是因果自注意力，得到意图嵌入，以及这个部分竟然用了类似TRM的递归循环模块+动态路由决定停止，难道这个在大模型领域真的已经非常普遍了？
  - （GPR）意图嵌入作为KV，送到新的交叉注意力网络，计算出思考tokens，然后sumpooling聚合，利用类似扩散的思路把思考tokens当condition，进行一次精炼
  - 用这个思考token和精炼token进行生成得到下一个item表示，并且利用多种指标进行优化
  - 引入MTP，预训练就用聚合的方式进行结果聚合，推理感觉就可以并行解了（N=4），同时利用某个确定的价值指标（eCPM）对loss权重进行加权
  - 预测结果，计算各种结果的加权和，但是并不止如此，还要层次过程奖励（HEPO）：通过计算用户的历史成功交互，就能推导出每个令牌的流行度（在积极结果中的出现频率），从而得到每个令牌的分数
  - 后面使用广义优势估计（GAE）来计算优势（看不懂）
  - 对用户进行数据合成来追随动态兴趣，有机令牌事实根据用户行为调整，环境令牌根据广告周边情况调整
  - 结果挺好，最有趣的可能是他提出的HEPO？感觉非常适合推荐系统
- 8. NIPS'25, [Language Ranker: A Lightweight Ranking framework for LLM Decoding](https://zhouchenlin.github.io/Publications/2025-NeurIPS-Language.pdf)
  - 不再依赖大型奖励模型，直接服用主模型提取的隐藏层特征完成重排序（对标超越传统奖励重排序，也就是著名的拒绝采样，先探索多条路径，再借由奖励模型来决定路径之间的优先级，起源于著名思路“验证比生成容易”）
  - 一个小发现☝️，最终层提取的特征可能不如底部60%处提取的特征效果好
  - 两种解决措施，但是差不多，如果是列表排序，就对中间特征和各个响应联合做transformer来混合建模，最终产出embedding再进行相似度计算；如果是点对点的就各自去过MLP，然后算相似度。（怎么感觉这俩都把特征和响应看成了单独的embedding？能这样做吗？）
    - 关于相似度计算，如果标签是01二元的就余弦相似度，如果是每个标签都有得分的话就用可学的
    - 关于优化，
      - 对于列表排序，如果结果是01分类，计算出每个响应对应的归一化标签和归一化分数，计算KL散度；如果结果是回归问题，进行MSE
      - 对于点对点排序，如果结果是01分类就交叉熵损失，如果结果是回归就MSE
  - 效果，提升！reward的对象是Llama3.1-8B-Instruct、Qwen2.5-7B-Instruct、Qwen2.5-32B-Instruct，实验数据集是MATH、MAPP、xLAM
  - 在重排序阶段，似乎也随着candidates提升展现出一定的scaling，numblock好像最大也就开到4，而且已经有下降趋势了
  - 似乎在单一任务上训练排序器，对后续别的任务也是有提升的
- 9. [From Scaling to Structured Expressivity: Rethinking Transformers for CTR Prediction](https://arxiv.org/pdf/2511.12081)（阿里）
  - 工业CTR任务直接使用LLM架构难以适配，代价越来越大，但是效果上升有限
  - 希望能够不要共享注意力，而是为每个field设计一个注意力：
    - 首先给每个field设计一个W_Q, W_K，这样就是$O(Fd^2)$，然后设计一个额外的$w_{f_i, f_j}$标量（注意这里的ij有前后顺序），这样虽然有$O(F^2)$的复杂度但也还可控（总算小于$O(F^2d^2)$）
    - 通过共享基矩阵和使用不同的轻量MLP生成权重的方式，高效存储注意力矩阵（应该是注意力矩阵）
- 10. AAAI'26, [Align³GR: Unified Multi-Level Alignment for LLM-based Generative Recommendation](https://arxiv.org/abs/2511.11255v1)（快手）
  - 专门进行LLM4Rec与语义编码的对齐（下面所有记述的SID，在文中似乎都被简称为SCID（Semantic Collaborative ID）
  - 语义编码生成又有了新方法，把语义embedding和cf embedding放在一起训
  - 文本推SID，SID推文本的经典对齐训练
  - 稀疏的反馈信号进行强化学习->效果差，因此设计一个从易到难的训练过程
    - 首先是渐进式SP-DPO，似乎正样本是真实next SID，但是负样本是当前策略函数生成的，似乎是prefixx n-gram决定难易度（其实就是看从第一位开始有几位code能匹配，匹配的越多越难），并且这个是渐进式的，也就是说第一次拿简单负样训完，就用训完的模型替代策略模型进一步生成难样本？
    - 然后是渐进式RF-DPO，似乎构建负反馈的方式依据用户的行为来，简单负样本是强负反馈（dislike），困难副样本是弱负反馈（not click）
- 11. NIPS'25, [Who You Are Matters: Bridging Topics and Social Roles via LLM-Enhanced Logical Recommendation](https://arxiv.org/pdf/2505.10940)（快手）（TagCF）[Code](https://github.com/Code2Q/TagCF)![alt text](images/image.png)
  - 要理解，这是一篇复古的LLM增强推荐的工作，代码甚至基于RecBole
  - Stage 1: 使用MLLM进行特征提取，得到物品主题和用户角色tag（物品的主题，以及物品是为谁准备的）
  - Stage 2: 大模型对齐，使用prompt，基于用户tag猜测相关的物品tag；基于物品tag猜测用户的类型。构建成U2I图谱和I2U图谱。
    - 额外地，使用精炼方式，将原始标签总结成较小的标签集
    - 大模型生成超高质量标签，然后使用轻量的蒸馏模型处理全量数据集
  - Stage 3: 
    - 1. 基于tag进行编码，对于item，都能找到item对应的物品tag和猜出的用户tag，因此用户进行序列建模的时候，不仅可以用纯id建模，也可以用tag拼一条序列出来，做序列建模，这样建出来的模之间就可以对比学习
    - 2. 物品和tag的表征之间肯定可以对比学习啊
    - 3. 基于用户历史可以得到推荐分数y，但是基于tag可以也得到另一个推荐分数，而且基于逻辑图谱还能探索新的用户tag，从而利用新tag得到另另一个推荐分数，这仨加权和为总推荐分数（但是不探索就已经有一定涨点了，不如说加了只有微涨）
  - 正如评论区兄弟们所说，这种标签基建还用得着你来做吗？中台早就做了。
- 12. ACL'24, [LayerSkip: Enabling Early Exit Inference and Self-Speculative Decoding](https://arxiv.org/pdf/2404.16710)(Meta)![Comparison between autoregressive decoding, speculative decoding, and our proposed self-speculative decoding.](images/image-1.png)
  - 利用llama-7B发现，前面的层就能预测对的token无关紧要，重要的token是偏后面的层，因此可以通过Layer-skip来降低计算量。然而，发现32层LLM好像平均要23层才能大致收敛，还能不能更快呢？
  - 1. 用一个特殊的loss来将早退层（其实也就是中间每层）的结果接到LM Head上，预测损失（加入正则化让所有层权重和为1）
  - 2. 自投机解码，前E层用来当draft model，完整模型L层当verification model（这样好处是其实verify的时候只需要L-E层）
- 13. arXiv'25, [NEZHA: A Zero-sacrifice and Hyperspeed Decoding Architecture for Generative Recommendations](https://arxiv.org/pdf/2511.18793)(阿里)（人大）
  - 总的来说，decoder阶段是最耗时的（超过60%），面对GR所需的超短序列生成，KV Cache效果一般，投机解码(Speculative Decoder(SD))还要额外验证，反而得不偿失，需要专门的解码技巧（不过还是改进投机解码）
  - 设计prompt的时候预先留好L个位置的空（对应L位rq code），从code_a的位置开始，用对应的lm head头解出来token之后不送到开头，而是直接用一个transition网络（RNN）直接结合刚刚解出来的占位符信息送到下一个lm head（也就是说这个lm head可以专门为rq code设计？）
  - 后面用规则手段进行筛选（就这？）
  - 训练过程也是沿着这个进行，只是中间的每一步transition的时候都使用ground truth作为输入；类似于正常训练，对所有位置的token作CE loss。
  - 感觉是基于RNN改进版的MTP，至于规则约束，别逗你TIGER哥LETTER哥笑了
- 14. arXiv'25, [ReaSeq: Unleashing World Knowledge via Reasoning for Sequential Modeling](https://arxiv.org/pdf/2512.21257v1)(阿里)
  - 问题：解决推荐系统的共现信息还是太稀疏（缺乏语义信息）、平台日志只能记录用户的一侧（平台记录用户行为，但是无法记录其他平台/真实世界其余行为）
  - 使用多智能体写作的方式显式推理出语义知识，然后语义编码
    - 基础层，建立全面的分类法，为后续推理阶段提供支撑
      - 用户需求导向流：分析每个类别的历史用户查询和搜索模式，执行语义聚类和需求分组，似乎是用来生成用户特征的
      - 产品属性导向流：部署一个属性提取agent，处理商品属性信息，感觉像输入商品属性文本，重写成tag
    - 维度细化层，对一层生成的类别进行细化，根据传入的类别上下文进行细化，保证正交性、全面性和客观性。（同时在这里使用了思维链来提升质量）
    - 项目特定知识生成层：根据用户需求，描述如何满足用户需求；根据物品属性，推测使用场景和风格特征等
  - 检索：直接利用user历史行为做余弦相似度；
  - 太长了，不想看了，是一篇技术报告
- 15. [OpenOneRec Technical Report: An Open Foundation Model and Benchmark to Accelerate Generative Recommendation](https://arxiv.org/pdf/2512.24762)(快手)
  - Main Contribution：
    - RecIF-Bench：包含8个任务，9600万条交互数据
    - OneRec-Foundation：基于Qwen2.5构建1.7B和8B模型
    - 完整Framework：发现增加训练数据量比模型什么的都重要
  - Method
    - 预训练：
      - 首先只训练embedding部分，做对齐任务；
      - 然后训练全参数，用推荐和通用混合数据训练
    - 后训练：
      - 首先指令微调，学会“推荐”和“解释”
      - 然后通用能力蒸馏，再让模型利用策略梯度法，在生成通用文本时对齐回原本的Qwen分布
      - 最后推荐强化学习，GRPO+Reward
- 16. ICML'24, [Medusa: Simple LLM Inference Acceleration Framework with Multiple Decoding Heads](https://arxiv.org/pdf/2401.10774)
  - 解决大模型自回归推理慢的问题，同时避免设计草稿模型进行投机采样（
  - 解决方法：似乎使用了类似MTP的推理机制，同时用k个头预测接下来的k步
  - 也就是说搜出的结果是树状的结构，利用特殊的mask机制把这个树展平，让大模型同时对所有的预测结果进行概率计算，相当于同时验证了所有可能结果
  - 后续/相关工作：
    - EAGLE：把t+1位置的预测token embeding也输给t+2位置的头
    - Sequoia：根据当前的预测置信度动态调整生成树的形状
    - Hydra：用RNN生成
- 17. arXiv'26, [Parallel Latent Reasoning for Sequential Recommendation](https://arxiv.org/pdf/2601.03153)
  - 从大模型分析来看，似乎只做了两件事情：
  -  1. sasrec生成一个user embedding
  -  2. 这个embedding分别加上不同的标记开启不同的生成（感觉就是refine）了一下，每条路径都进行了一段latent reasoning，最终结果用kl散度保证各条路径不同（这和moe的解码有什么区别）
- 18. AAAI‘25, [Enhance Vision-Language Alignment with Noise](https://arxiv.org/pdf/2412.10817)(PiNI)
  - 解决CLIP中的在少样本下游任务上难以对齐的问题
  - 给训练和推理的图片加一个特殊噪声，这个噪声可学均值方差，然后将这个均值方差施加到正常高斯分布得到针对embedding具体位置的细粒度噪声
  - 然后发现这样一弄效果提升了

#### 20250722 现在调研一下组会要讲的ppt
1. [DeepRec: Towards a Deep Dive Into the Item Space with Large Language Model Based Recommendation](https://arxiv.org/pdf/2505.16810)
  - 已有的LLM+RS的工作主要集中在大模型增强feature用于推荐、推荐生成candidate用于大模型推理、大模型直推和大模型agent部分，这里改为多轮推理检索
  - 每一轮交互，LLM都会根据历史交互和上一轮检索的item生成偏好描述，从而让推荐再根据偏好检索相关项目，从而进行多轮交互
  - 基于推荐模型的数据回放：利用用户偏好和历史交互融合成用户表示，然后LLm沿着推理-检索范式进行交互，最后根据推荐结果找到难度适中的训练数据。
  - 奖励包含格式奖励、调用推荐次数奖励、偏好多样性奖励、预测结果质量奖励、项目列表奖励
  - 过程奖励引导LLM与推荐系统进行交互，并且在此基础上利用结果奖励提高质量
2. [Reason-to-Recommend: Using Interaction-of-Thought Reasoning to Enhance LLM Recommendation](https://arxiv.org/pdf/2506.05069)
  - 利用用户-项目图构建u1-i1-u2-i2-u1思维链，用这个链充当推理链鼓励大模型思考
  - 进行监督微调引导LLM自主生成推理链
  - 强化学习帮助可扩展的监督
3. [Reinforced Latent Reasoning for LLM-based Recommendation](https://arxiv.org/pdf/2505.19092)[Code](https://anonymous.4open.science/r/R3-A278)
  - 想要在推荐中利用到大模型的推理能力，就面临以下问题：
    - 高质量CoT数据难以获取
    - CoT延迟过高
    - 现实中根本没有所谓“CoT”数据
  - 在LLMs最终解码层的顶端增加额外的注意力层，来从隐藏层中提取信息，将信息转化为输入嵌入空间对齐的隐推理标记
4. [RecLLM-R1: A Two-Stage Training Paradigm with Reinforcement Learning and Chain-of-Thought](https://arxiv.org/pdf/2506.19235)（小红书）
  - 数据构建
    - 用户侧取出画像，将历史行为按照主题聚合
    - 物品侧利用描述、标题、基于内容的细粒度标签和统计特征作为自然语言输入
    - 基于用户参与度指标构建偏好序列作为正样本
    - 用户与物品侧内容进行拼接构成数据
  - SFT+GRPO+CoT


#### 20250616 还是开始看看LLaMA-Factory叭
- arXiv'25, [OneRec Technical Report](https://arxiv.org/pdf/2506.13695)
  - 当前的传统推荐pipeline存在几个问题：
    - 计算通信代价很大，导致对高性能显卡利用率低下
    - 目标冲突，多任务冲突以及各个阶段之间的冲突
    - 发展落后，别人都在猛猛各种新方法，scaling和RL，但是我们现在缺少这些架构
  - Tokenizer
    - 利用QFormer的cross attention来对齐多模态信息，从多模态内容中得到压缩版本的多模态表示
    - 利用协同相似性和共现相似度等方式在序列内外匹配构建item-item对
    - 最小化ii对之间的距离的loss，同时增加caption next token prediction loss来避免幻觉
    - 然后残差量化，他认为RQ-Means效果超过传统RQ-VAE
  - Encoder
    - 从特征、短期交互、正反馈交互、终身行为压缩（聚类）的形式来获得表征
    - 表征拼合送入encoder过自注意力
  - Decoder
    - 标准的encoder-decoder架构的decoder，CausalSelfAttn+和Encoder的CrossAttn+MoE-RMSNorm
  - 奖励系统
    - 本来明明是对每个目标进行BCE的单塔（比如点击、喜欢等）（当然这些任务loss也存在），但是现在将这些输出和用户信息等融合后再输出一次（还是计算所有标签BCE），这样能融合用户信息
    - 这么做是因为我们生成的本质还是一个item召回，我们算不出item分数，所以我们使用SIM作为这个P-Score打分器来生成分数（因为我们输入SIM的已经是ui对了）
    - 原本引入KL好像是为了避免丢失能力（变化过大），但是这里的强化学习去掉KL可能就是因为SFT和RL同时进行，本身已经不需要稳定
    - 格式奖励和对齐奖励其实挺好理解的，格式奖励保证生成合法token，工业奖励可以随时根据需求进行调整（一般都是一个规则，感觉可以视作引入软规则）
  - 训练
    - 预训练：onerec-0.935b在10^11个item训练后收敛，看上去还是i太大了
    - 后训练：采样播放时长前50%，同时强化学习是分开到后方的的，保证实时性
    - 差不多都是10^10个样本后收敛，但是能够一直到10^11个样本才完全收敛
  - 强化学习实验情况
    - 似乎RQ更适合Beam Search，不过这个大家也都知道了
  - RQ这个真是太奇怪了，为什么RQ-Means会取得比RQ-VAE好这么多的效果？难道真的是因为稳定？
    - 但是看得出来这个根据协同+语义相似寻找相似样本的方式确实很好，比单一的强多了
- arxiv'25, [OneRec-V2 Technical Report](https://arxiv.org/pdf/2508.20900)(kuaishou)
  - 使用lazy decoder-only来代替过于占据资源的encoder-decoder架构
    - 直接消除encdoer，只保留decoder，将用户的历史行为、画像等上下文信息作为静态条件存下来，然后解码器可以用cross-attention来访问
    - 改进交叉注意力，取消对KV的投影层，多个解码器模块共用一组从上下文信息中提取的KV对（本质上是为每层设计了一对KV）
  - 更新强化学习
    - 首先将视频长度分桶，然后就可以为每个视频确定属于哪一个分组，通过视频的实际播放长度就能得到视频的分位数，只有在前25%分位数才是正样本，另一方面，产生负反馈就是负样本
    - ECPO针对负样本进行了一定程度的梯度截断，但是ECPO对负样本还是会梯度爆炸
    - clip的本意是让策略不要沿着当前方向（比如正样本就是预测概率变大，负样本就是预测概率变小）前进得太多，以免过拟合，所以A>0的时候对变大截断，A<0的时候对变小截断
    - 但是在负样本(A<0)的情况下，如果预测概率本身比较小会导致梯度变得过大（见求导公式），这就使得模型走向崩溃（比如过拟合或者梯度爆炸）；但是bce损失就没这个问题，因此模仿bce，预测概率越小，就越严格限制奖励的上限。
    - 原本的GRPO，如果正样本的预测概率过大，或者负样本的预测概率过小（这两个“过”都是相对老模型而言），就不会更新了，但是现在修改之后，负样本如果变好得比较猛烈（预测概率猛降），还是会更新，这是为了提升数据利用率，但是如果在变坏（预测概率增），则和grpo的不截断以及ecpo的固定截断不同，会根据预测概率的大小进行动态截断（预测概率越小，变化会越剧烈，所以截断也会越早）
- arXiv'25, [OneSug: The Unified End-to-End Generative Framework for E-commerce QuerySuggestion](https://arxiv.org/pdf/2506.06913)(kuaishou)
  - 还是onerec的后续工作，不过这里进行的任务是query suggestion（搜索词推荐）
  - motivation
    - 上个模型的性能会限制下一阶段
    - 不同阶段模型的目标不一致，导致整体级联架构次优
    - 对长尾候选项表现次优
  - 其实非常类似于推荐系统，我们其实是要从候选query池中召回适合的补全，首先我们要拼一个输入，输入是四个部分：输入的前缀内容、召回的相近query的rq表示（总之这次召回用了个什么方法找到了相近的query和高质量的query，不知道怎么做的）、历史query序列、user profile
  - 针对DPO制作正负对，构建了一些正负对
  - 一个优化的DPO loss来捕捉更细粒度的奖励信息，具体而言list-wise loss，本质上是将一个正样本和多个负样本之间奖励差加在一起。
  - 额外实验表明，这个做长尾特别的好
- SIGIR'25, [Killing Two Birds with One Stone: Unifying Retrieval and Ranking with a Single Generative Recommendation Model](https://arxiv.org/pdf/2504.16454)(huawei)
  - 检索和排序放在同一个模型内
  - 这样同时预测next item的点击概率和下一个可能交互的物品
  - 利用排序阶段的精确性找到检索阶段难以区分的负样来提升检索训练
  - 梯度自适应权重，来平衡两个阶段
...
- arXiv' 2025, [Soft Thinking: Unlocking the Reasoning Potential of LLMs in Continuous Concept Space](https://arxiv.org/pdf/2505.15778)
  - 对于大模型，不直接采样得到下一个输入，下一个输入是每个位置的embedding乘概率（也就是说是隐式输入）
  - 似乎是专门用于cot部分的，然后根据熵值决定是否提前终止推理
- arXiv'25, [REG4Rec: Reasoning-Enhanced Generative Model for Large-Scale Recommendation Systems](https://arxiv.org/pdf/2508.15308)
  - 动机
    - 过去基于语义编码的推荐，虽然也有多步推理来逐渐构建item，但是每个item被编码成固定的tokens，这和多样化的推荐路径不匹配
    - 当前的“推理”方法其实缺少中间过程的评估
  - 做法
    - 使用一个类似MoE的方式来进行MPQ，可能跟传统PQ的区别就是使用了多套encoder？然后分别进行VQ，最后拼合回到一个Decoder中...哦我懂了，PQ的所有子向量是正交的，这个不是，这个是用多个并行encoder生成的
    - 也就是说，这使得推理的时候可以动态从所有并行code中进行选择，也就是说一个item都可以用k个潜在表明来表示，这k个潜在表明是平行的，生成的时候会同时生成k个结果，但是这是乱序的，也就是说每次找到置信度最高的某个位置的码本，找到对应之后从候选码本中去掉这个，然后重复这个过程。
    - 预训练-微调范式，预训练的时候就正常类似tiger进行预训练，似乎是自回归；然后微调的时候使用GRPO，首先是Step-Hit奖励，这个是生成的token和真实item的对应码匹配的奖励，然后是Step-Consistency奖励，是利用JS散度度量第i步和第i+1步之间的距离，越小越平滑越好，最后是Global Path奖励，对于长为M的推理路径，使用M-d个正确token构建累加和，看看能不能检索到对应item，能的话为1
    - 为了“推理”，对上面的step和path进行了扩增，可以计算多步（多个item）的损失，并且使用权重系数来衰减，但感觉没啥用
    - 增加自反思机制，因为辅助任务会生成一个item类别的分布，如果前后类别js散度太大，那就会终止当前路径。
- arXiv '25, [OnePiece: Bringing Context Engineering and Reasoning to Industrial Cascade Ranking System](https://arxiv.org/pdf/2509.18091)(人大)(Shopee)
  - 结构化上下文，似乎包含了交互历史、群体辅助商品、场景描述、候选商品集 这四块，但是具体的没细看
  - 块推理...怎么完全看不懂
...
- KDD26审稿, [Building Global Patent Clusters and Mapping the Innovation Landscape]
  - 给出的代码非常完善
- WWW ADS26审稿, [HoMer: Addressing Heterogeneities by Modeling Sequential and Set-wise Contexts for CTR Prediction]
  - 解决CTR中特征异质性（序列特征少而非序列特征多）、上下文异质性（逐点预测忽略了物品之间的兴趣影响）、架构异质性（各种网络集成碎片化）
  - 说实话我竟然看不出什么新意，难道新意就是输入模型的时候，把user profile和item feature，context feature什么的对齐？
  - 哦还有encoder、decoder，decoder的时候对整个候选集直接进行decoder，这样并行效率高？同时还能计算候选集内部item间的相互作用。
- TKDD审稿, [Dual-Stream MLP is All You Need for CTR Prediction]
  - 似乎是用双流MLP进行CTR预测（考虑类似FinalMLP）
  - 现有问题：特征交互复杂，显隐模块不平衡。
  - 利用知识蒸馏把学习显式交互的能力蒸馏到主MLP模型中
  - 就完了？
  - 把显式交互的那部分单独蒸馏到一个MLP中，然后用那个MLP代替显式交互的部分...就完了？
  - simple，but not effective
- ArXiv, [OneTrans: Unified Feature Interaction and Sequence Modelingwith One Transformer in Industrial Recommender](https://arxiv.org/pdf/2510.26104)(字节)
  - 一个统一模型，能够同时建模用户序列和特征交互
  - 请看主框图，主框图中的c部分Mix Parameterization有很明确的描写，对于序列信息内部是共享权重的因果掩码注意力，而对于非序列信息则是每个部分单独有自己的注意力权重。但是非序列token仍然能够看到完整的序列token
  - 预处理部分，总之能产生几个非序列token，首先全部拼在一起，利用MLP处理成Lxd的形式，然后切分成L个
  - 预处理部分，序列token是按照时间戳有序排列，并且用适当的token来指示行为类型
  - S token：Sequence token；NS token None-Sequence token
  - 金字塔调度：在每个block中，只有一部分S token发出查询，查询全部的输入的历史token，这样每次block后，查询集都会变小，形成一个类似蒸馏的方法
  - 因为每个候选项似乎有自己的NS token，故只能各自去算，所以使两阶段的，第一阶段计算S端注意力，第二阶段对不同候选项计算NS端，由于此时S端已经固定，就可以让NS端均摊S端损失
  - 不使用因果注意力/不使用金字塔调度，竟然并没有什么太大影响（
- ArXiv '25, [MTmixAtt: Integrating Mixture-of-Experts with Multi-Mix Attention for Large-Scale Recommendation](https://arxiv.org/pdf/2510.15286)(美团)
  - 现有问题：特征工程中的特征组构建、不同模态架构异构、跨场景
  - 特征组构建，首先利用DNN进行维度对齐，然后训练一个特征选择模块进行选择，利用argmax-k进行选择，可以选择出n_g个组，每个组有k个特征
- TOIS'25审稿, UniRec: Unified Space Learning for Generalizable Sequential Recommendation
  - 解决LLM序列推荐的限制，首先是LLM产生幻觉，然后是跨域推荐容易产生转移不良
  - 本文就旨在结合文本的跨域序列推荐中提升序列表征质量
  - 具体方法
    - Stage 1: 统一空间预训练
      - 首先进行类似推荐任务的指令微调（输入序列，输出target item）
      - 然后基于真实空间构造前缀树保证生成的绝对是真实item。因为前缀树只在推理的时候发挥作用
      - 为了避免训练和推理时的不一致，在训练的时候仅允许前缀树中合法的token进行softmax（这和将前缀树也应用到训练阶段有什么区别？）
      - 利用前缀树进行了向量压缩——既然已经有前缀树进行约束，那么合法ID相比非法ID来说数量级远远地小，因此可以仅记录合法ID，压缩成一个单独的数组来记录那些IDs
    - Stage 2: 跨域自适应转移
      - 对语言模型生成的embedding进行奇异值分解来降维
      - 实际构造的时候，再额外用推荐系统生成一个cf embedding，用一个门控网络决定这个cf embedding该以何等权重拼在语言embedding后面。
  - 实验
    - 缺
- ICML'26 审稿, DRIFT: Decoupled Rollouts and Importance-Weighted Fine-Tuning for Efficient Multi-Turn Optimization
  - 多轮LLM的优化容易陷入二选一的困境（SFT高效但是效果差；RL效果好但是成本高）
  - 可以将轨迹生成和策略优化完全分开，变成离线轨迹采样+重要性加权SFT，能这样做的背景是论文试图证明KL散度loss是一种特殊的重要性加权SFT，我不确定，不审了这篇。
- ICML'26 审稿, Offline Preference Learning with Clustering and Active Data-Augmentation
  - 用来研究LLM中的用户偏好识别（我觉得灵感来自于推荐系统，但是文章自称是泛用的）
  - 通过聚类引入相似用户，能够提高对用户偏好的学习；通过最小特征值增量来决定是否混合更多的用户，进一步提升性能。
  - 非常重要的是通过证明证明了这一点
